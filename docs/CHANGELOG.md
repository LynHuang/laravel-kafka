...
**事务使用决策 + Consumer 端幂等性最佳实践**——业务方两次关键反馈：

1. **单节点应用应避免事务**——同一 producer 只能跑 1 个事务 + ~10ms init 开销，单节点部署多个进程会冲突
2. **Consumer 端必须做幂等性**——Kafka 事务只在 Producer 端原子；Consumer 端重试 / 进程崩溃会重复消费

#### 03 §7.5 加 "何时用 vs 何时不用事务" 决策表（v0.5.6 新增 ~60 行）

- 6 类业务场景对应表（单节点 / 微服务 / 多 topic 原子 / 单 topic / 批量等）
- **单节点应用应避免事务的 4 个具体原因**（Mutex 串行 / init 开销 / transactional_id 冲突 / 双层事务嵌套）
- 多节点部署运维注意（`${POD_NAME}` 后缀 / 滚动升级 epoch 释放）

#### 04 §1.6 加 §9 消费端幂等性最佳实践（v0.5.6 新增 ~180 行）

- **核心警告框**：事务 + Consumer 必修（重试机制 + 进程崩溃会重复消费）
- **3 个重复来源**：进程崩溃 / 重试机制 / librdkafka 内部 retry
- **幂等 key 设计原则**：业务主键（订单号 / 支付单号），不要用 timestamp / uuid
- **3 种实现方式**：
  - 方式 A：DB 唯一索引（强一致，适用资金 / 订单）
  - 方式 B：Redis SETNX（高性能，适用通知 / 分析）
  - 方式 C：业务表 UNIQUE 约束（最省事）
- **选型决策表**（按业务重要性分）
- **完整端到端示例**（Producer 事务 + Consumer 幂等）
- **真实事故案例**（超卖纠纷 → 修法）

#### 验证

- **0 代码变更**——纯文档 release
- 文档与 v0.5.4/v0.5.5 一致风格
